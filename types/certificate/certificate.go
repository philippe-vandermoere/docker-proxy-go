...
const certificatesDirectory = "/etc/nginx/certificates"
...
func (certificate Certificate) GetFileName() string {
	return certificatesDirectory + "/" + certificate.Domain + "/" + "certificate.pem"
}

func (certificate Certificate) GetPrivateKeyFileName() string {
	return certificatesDirectory + "/" + certificate.Domain + "/" + "privateKey.pem"
}

func (certificate Certificate) Write(certificateContent string, privateKeyContent string) error {
	err := os.MkdirAll(certificatesDirectory+"/"+certificate.Domain, 755)
	if err != nil {
		return err
	}
