...
import (
	...
	"github.com/philippe-vandermoere/docker-proxy-go/docker"
...
func getServer(container typeDocker.Container) (typeServer.Server, error) {
	...
	container, err = docker.NetworkConnect(proxyNetwork, container)
	if err != nil {
		return server, err
	}
...
func applyConfig() error {
	...
	result, err := docker.ContainerExec(nginxContainer, []string{"nginx", "-s", "reload"})
	if err != nil {
		return err
	}
